...
CUDA_VERSIONS = ["12.4", "12.6"] if sys.platform != "darwin" else []  # Chỉ build CUDA cho Monitoring Unit, bỏ qua trên macOS
...
def install_pytorch(cuda_version):
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "uninstall", "-y", "torch", "torchvision", "torchaudio"], check=True
    )
...
def main():
    ...
    # Build Client App (Không cần CUDA)
    build_app("Client", SOURCE_DIRS["Client"])

    # Build Monitoring Unit App (Bản mặc định, không CUDA)
    build_app("Monitoring Unit", SOURCE_DIRS["Monitoring Unit"])

    # Nếu không chạy trên macOS, tiến hành build với CUDA 12.4 và 12.6
